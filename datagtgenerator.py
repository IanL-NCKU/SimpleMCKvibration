...
def main():
    n = 100000
    output_filename = 'newsmall_scale_test_vibration_data.npz'
    output_folder = 'newsmall_scale_test_data_distributions'
        ...
        with warnings.catch_warnings(record=True) as w:
            ...
            with np.errstate(all='raise'):  # Raise exceptions instead of warnings
                try:
                    ...
                    elif any(np.abs([x_t, v_t, a_t]) < 1e-3):
                        if np.random.rand() > 0.01 or any(np.abs([x_t, v_t, a_t]) < 1e-4):
                            is_valid = False
                            rejected_small += 1
