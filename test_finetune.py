...
class MSELoss(BaseLossComponent):
    ...
    def compute(self, predictions, targets, inputs, norm_params=None, inputs_real=None):
        ...
        if self.use_log:
            # Extract signs and convert to class indices for cross-entropy
            # Sign values: -1, 0, +1 -> Map to class indices: 0, 1, 2
            sign_predictions = torch.sign(predictions)
            sign_targets = torch.sign(targets)

            # Map signs to class indices: -1→0, 0→1, +1→2
            sign_pred_classes = (sign_predictions + 1).long()  # Shape: [batch, 3]
            sign_target_classes = (sign_targets + 1).long()    # Shape: [batch, 3]
            ...
            # Compute sign cross-entropy loss
            # Flatten to (batch_size * 3,) for cross-entropy
            sign_pred_flat = sign_pred_classes.view(-1)      # Shape: [batch*3]
            sign_target_flat = sign_target_classes.view(-1)  # Shape: [batch*3]

            # Create one-hot encoding for predictions (num_classes=3)
            num_classes = 3
            sign_pred_onehot = F.one_hot(sign_pred_flat, num_classes=num_classes).float()  # Shape: [batch*3, 3]

            # Scale one-hot to logits (larger scale = more confident predictions)
            sign_pred_logits = sign_pred_onehot * 10.0  # scale by the number of size(target)[1]

            # Compute cross-entropy loss
            sign_ce_loss = F.cross_entropy(
                sign_pred_logits,    # Predictions as logits
                sign_target_flat,    # Ground truth class indices
                reduction='mean'
            )

            # Combine magnitude and sign losses
            loss = magnitude_loss + sign_ce_loss
        else:
