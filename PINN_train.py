...
def save_model_params(model, folder, loss_config, extra_info=None):
    """Write a human-readable params.txt with model architecture and parameter counts."""
    lines = []
    lines.append("=" * 60)
    ...
    subnetworks = {
        'backbone (network)':   getattr(model, 'network',               None),
        'finetune_network':      getattr(model, 'finetune_network',      None),
        'logabs_sign_network':   getattr(model, 'logabs_sign_network',   None),
        'real_sign_network':     getattr(model, 'real_sign_network',     None),
    }

    total_params    = 0
    total_trainable = 0
    for name, net in subnetworks.items():
        if net is None:
            lines.append(f"  {name:30s}: not present")
            continue
        ...
        lines.append(f"  {name:30s}: {n_train:>8,} trainable  /  {n_all:>8,} total")

    lines.append("-" * 60)
    lines.append(f"  {'TOTAL':30s}: {total_trainable:>8,} trainable  /  {total_params:>8,} total")
    lines.append("")

    lines.append("Model flags:")
    for attr in ('use_internal_sign', 'use_finetune', 'activation'):
        val = getattr(model, attr, 'N/A')
        lines.append(f"  {attr}: {val}")
    lines.append("")

    lines.append("Loss config:")
    for loss_name, cfg in loss_config.items():
        lines.append(f"  {loss_name}: {cfg}")
    lines.append("")
    ...
    out_path = os.path.join(folder, 'params.txt')
...
def main_ver0():
    ...
    # Data paths
    Train_Val_data_source = r'Physic_relate_train_val_vibration_data.npz'
    Test_data_source = r'Physic_relate_test_vibration_data.npz'
    Plot_data_source = r'Physic_relate_test_vibration_data.npz'
    # Load the dataset
    ...
    loss_config = {
        ...
        "InitialCondition": {"weight": 0.0, "use_log": True, "use_relative": True, "t_threshold": 1e-6},  # Disabled for now
        "Consistency": {
            "weight": 0,
            "t_threshold": 1e-6,
            "use_log": True,
    ...
    save_model_params(model, model_and_result_folder, loss_config, extra_info={
        'epochs': epochs,
        'device': str(device),
        'dtype': str(dtype),
    })
        ...
        with context_manager:
            ...
            for inputs, targets in val_pbar:
                ...
                if loss_fn.has_loss("MSE"):
                    ...
                    loss_args["MSE"] = (
                        mag_preds, targets,
                        logabs_sign_probs, None, None,
                        real_sign_probs, ft_cal,
                        train_val_targets_normalizer
                    )
                if loss_fn.has_loss("Residual"):
                    logabs_sign = SignWithHardTanh.apply(logabs_sign_pred)
                    signed_mag_preds = logabs_sign * mag_preds
                    outputs_for_residual = torch.cat([
                        real_sign_pred,
                        signed_mag_preds + ft_cal
                    ], dim=1)
                    loss_args["Residual"] = (
                        outputs_for_residual, targets,
                        inputs_real,
                ...
                if loss_fn.has_loss("Consistency"):
                    ...
                    if consistency_type == "auto":
                        ...
                        raise ValueError(f"Consistency type '{consistency_type}' not supported in validation. Use 'auto'.")
                if loss_fn.has_loss("InitialCondition"):
                    loss_args["InitialCondition"] = (inputs, inputs_real, ft_cal)
        ...
        for inputs, targets in test_pbar:
            ...
            if loss_fn.has_loss("Consistency"):
                ...
                if consistency_type == "auto":
                    ...
                    raise ValueError(f"Consistency type '{consistency_type}' not supported in test. Use 'auto'.")
            if loss_fn.has_loss("InitialCondition"):
                loss_args["InitialCondition"] = (inputs, inputs_real, ft_cal)
...
def main():
    ...
    train_in_64 = True
    epochs = 200
    plot_only = False  # Set True to skip training and just regenerate plots
    ...
    model_file_name = 'vibration_model_test_newinitial.pt'
    model_and_result_folder = './vibration_model_test_newinitial'
    ...
    # Configure losses
    loss_config = {
        "MSE": {"weight": 0.7, "use_relative": False, "use_log": True, "sign_bce_weight": 1, "real_sign_bce_weight": 1, "ft_cal_weight": 1},
        "Residual": {"weight": 0, "use_relative": True},
        "InitialCondition": {"weight": 0.1, "use_log": True, "use_relative": True, "t_threshold": 1e-5},  # Disabled for now
    ...
    for epoch in range(epochs):
                ...
                if dx_prime_dt_prime is not None:
                    ...
                    if dv_prime_dt_prime is not None:
                        ...
                        if len(v_model_log) > 0:
                            print(f"Last batch logabs v_model v_theory_model v_target: [{abs(v_model_log[-1]):.8f} {abs(v_theory_model_log[-1]):.8f} {abs(v_target_log[-1]):.8f}]")
                            print(f"Last batch logabs a_model a_theory_model a_target: [{abs(a_model_log[-1]):.8f} {abs(a_theory_model_log[-1]):.8f} {abs(a_target_log[-1]):.8f}]")
        ...
        # Group 3: Initial Condition Loss
        if 'initial_condition_loss' in train_loss_components:
            initial_value = train_loss_components['initial_condition_loss']
            initial_ratio = (initial_value / train_total * 100) if train_total > 0 else 0
            train_groups.append(f"{{Initial_condition_loss: {initial_value:.4e} ({initial_ratio:.2f}%)}}")

        # Group 4: Consistency Loss
        ...
        # Group 3: Initial Condition Loss
        if 'initial_condition_loss' in val_loss_components:
            initial_value = val_loss_components['initial_condition_loss']
            initial_ratio = (initial_value / val_total * 100) if val_total > 0 else 0
            val_groups.append(f"{{Initial_condition_loss: {initial_value:.4e} ({initial_ratio:.2f}%)}}")
    ...
    # Group 3: Initial Condition Loss
    if 'initial_condition_loss' in test_loss_components:
        initial_value = test_loss_components['initial_condition_loss']
        initial_ratio = (initial_value / test_total * 100) if test_total > 0 else 0
        test_groups.append(f"{{Initial_condition_loss: {initial_value:.4e} ({initial_ratio:.2f}%)}}")
    ...
    # Group 3: Initial Condition Loss
    if 'initial_condition_loss' in verify_train_loss_components:
        initial_value = verify_train_loss_components['initial_condition_loss']
        initial_ratio = (initial_value / verify_train_total * 100) if verify_train_total > 0 else 0
        verify_train_groups.append(f"{{Initial_condition_loss: {initial_value:.4e} ({initial_ratio:.2f}%)}}")
    ...
    # Group 3: Initial Condition Loss
    if 'initial_condition_loss' in verify_val_loss_components:
        initial_value = verify_val_loss_components['initial_condition_loss']
        initial_ratio = (initial_value / verify_val_total * 100) if verify_val_total > 0 else 0
        verify_val_groups.append(f"{{Initial_condition_loss: {initial_value:.4e} ({initial_ratio:.2f}%)}}")
...
# ── Rung harness interface ──────────────────────────────────────────────────
# Translation layer between frozen rungs.py and model internals.
#
# Usage in rungs.py:
#   import train as train_mod
#   train_mod.init_normalizers(inputs_norm, outputs_norm)   # once after ckpt load
#   inputs   = train_mod.sampler_transform(m, c, k, x0, v0, t)
#   real_xva = train_mod.output_inverse_transform(model(*inputs))

_inputs_normalizer  = None
_outputs_normalizer = None
_out_alpha          = None   # (3,) cached generalized alpha for [x, v, a]
_out_beta           = None   # (3,) cached generalized beta  for [x, v, a]


def init_normalizers(inputs_norm, outputs_norm):
    """Inject checkpoint normalizers into module state."""
    global _inputs_normalizer, _outputs_normalizer, _out_alpha, _out_beta
    _inputs_normalizer  = inputs_norm
    _outputs_normalizer = outputs_norm
    _out_alpha = np.array([
        float(generalize_alpha(outputs_norm, outputs_norm.log_mean['x'], outputs_norm.log_std['x'], 'x')),
        float(generalize_alpha(outputs_norm, outputs_norm.log_mean['v'], outputs_norm.log_std['v'], 'v')),
        float(generalize_alpha(outputs_norm, outputs_norm.log_mean['a'], outputs_norm.log_std['a'], 'a')),
    ])
    _out_beta = np.array([
        float(generalize_beta(outputs_norm, outputs_norm.log_mean['x'], outputs_norm.log_std['x'], 'x')),
        float(generalize_beta(outputs_norm, outputs_norm.log_mean['v'], outputs_norm.log_std['v'], 'v')),
        float(generalize_beta(outputs_norm, outputs_norm.log_mean['a'], outputs_norm.log_std['a'], 'a')),
    ])


def sampler_transform(m, zeta, k, x0, v0, t):
    """Real-space (m, zeta, k, x0, v0, t) -> normalized model input 1-tuple.

    Dataset natively stores zeta (damping ratio); feed it directly.
    Returns (tensor,) so model(*sampler_transform(...)) unpacks cleanly.
    """
    device = m.device
    dtype  = m.dtype
    X_np   = torch.stack([m, zeta, k, t, x0, v0], dim=-1).detach().cpu().numpy()
    X_norm = _inputs_normalizer.normalize_inputs(X_np)
    return (torch.tensor(X_norm, dtype=dtype, device=device),)


def output_inverse_transform(raw_model_output):
    """(mag_preds, logabs_sign, real_sign, ft_cal) -> real-space (N, 3) [x, v, a].

    Phase-aware: ft_cal=None or all-zero -> Phase 1 (no calibration).
    Denormalizes via cached alpha/beta, reattaches real-space signs.
    """
    mag_preds, logabs_sign, real_sign, ft_cal = raw_model_output
    device = mag_preds.device
    dtype  = mag_preds.dtype
    ln10   = torch.log(torch.tensor(10.0, device=device, dtype=dtype))

    if ft_cal is None or torch.all(torch.abs(ft_cal) < 1e-12):
        ft_effective = torch.zeros_like(mag_preds)
    else:
        ft_effective = ft_cal

    signed_logabs = torch.sign(logabs_sign) * (mag_preds + ft_effective)  # (N, 3)

    alpha = torch.tensor(_out_alpha, device=device, dtype=dtype)  # (3,)
    beta  = torch.tensor(_out_beta,  device=device, dtype=dtype)  # (3,)

    log_vals = signed_logabs * beta + alpha   # (N, 3) denormalized log10
    mags     = torch.exp(log_vals * ln10)     # (N, 3) magnitudes
    return torch.sign(real_sign) * mags       # (N, 3) real-space [x, v, a]
